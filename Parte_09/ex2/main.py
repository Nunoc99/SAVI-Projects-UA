...
def main():
    ...
    plane_model, inlier_idx = point_cloud.segment_plane(distance_threshold=0.35,
                                         ransac_n=3,
                                         num_iterations=1000)

    a, b, c, d = plane_model
    point_cloud_inliers = point_cloud.select_by_index(inlier_idx, invert=False)
    point_cloud_inliers.paint_uniform_color([1,0,0])

    point_cloud_outliers = point_cloud.select_by_index(inlier_idx, invert=True)
