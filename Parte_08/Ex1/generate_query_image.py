...
def main():
    ...
    # Match the keypoints
    index_params = dict(algorithm=1, trees=15) # dicionário
    search_params = dict(checks=50) # dicionário
    flann_matcher = cv2.FlannBasedMatcher(index_params, search_params) # biblioteca que faz as associações mt rápido
    two_best_matches = flann_matcher.knnMatch(q_descriptors, t_descriptors, k=2)

    # best match é o tuplo de lista de tuplos, em cada tuplo estão, neste caso, 2 valores, pq k = 2
    ...
    for match_idx, match in enumerate(two_best_matches):
        ...
        if best_match.distance < 0.3 * second_match.distance:  # this is a robust match, keep it
            # 0.3 estamos a ser muito exigentes, o best match vai ser muito melhor do q os outros
