...
def main():
    ...
    best_matches = flann_matcher.knnMatch(q_descriptors, t_descriptors, k=1) # k = 1, means the best match, tuplos com k elementos, so melhores k matches
    ...
    for match_idx, match in enumerate(best_matches):
        ...
        # y_q = int(q_key_points[q_idx].pt[1])

        x_t, y_t = int(t_key_points[t_idx].pt[0]), int(t_key_points[t_idx].pt[1])
        # y_t = int(t_key_points[t_idx].pt[1])

        cv2.circle(train_img_gui, (x_t, y_t), 255, (0,0,255), 1)
        cv2.circle(query_img_gui, (x_q, y_q), 255, (0,0,255), 1)
